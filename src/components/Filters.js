...
const Filters = () => {
  const { filters: { text, category, company, min_price, max_price, price, shipping }, updateFilters, clearFilters, all_books } = useFilterContext();
  
  const categories = getUniqueValues(all_books, 'category')
  const companies = getUniqueValues(all_books, 'company')
        ...
        <div className='form-control'>
          ...
          <div>
            {categories.map((c, index) => {
              ...
              className={`${category === c.toString().toLowerCase() ? 'active':null}`}>
                {c}
              </button>
            })}
          </div>
